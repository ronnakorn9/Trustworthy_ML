...
def load_datasets(num_shards, save_directory):
    """
    Load list of dataset
    """
    data_list = []
    for i in range(num_shards):
        shard_filename = os.path.join(save_directory, f"shard_{i}.pkl")
...
def evaluate_majority_vote(models, test_loader, device):
    """
    Evaluate majority vote for list of model in test set
    Basically inference with list of model
    """
    correct = 0
    total = 0
    all_predictions = []
